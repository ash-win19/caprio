...
2. The API loads that day's messages, saved tasks, inbox, categories, state, version, the day's draft (as a computed `plan` view), and review.
...
5. The agent changes the day's draft only through planner tools (`add_task`, `edit_task`, `move_task`, `remove_task`, `set_completed`, `revert_change`, `read_plan`) and replies in plain text. Each tool call reaches the backend at `/internal/planner/tools/:name` with the shared `PLANNER_TOOL_SECRET` and a signed, short-lived turn token. Chat never mutates tasks.
6. The backend validates each tool call (owned task IDs, fields, category ownership, writable dates, duplicate titles against saved tasks and the draft), merges it into one keyed draft per user per day, and returns the full resulting plan. Rejections come back as `ok: false` so the model can correct itself within the turn.
7. A turn runs without a transaction open during the model call: begin (request ledger check, the day's turn lock, draft copied to `draft_staged`), the agent call (tools edit `draft_staged`), then one commit (user and assistant messages, a `plan_update` event listing the turn's changes, and the staged draft promoted). A failed, rejected, or stopped turn drops its staged changes. One turn runs per day at a time; a concurrent message gets `turn_in_progress`, and a retry of the same request waits for its result.
8. Confirmation checks the draft ID, workflow version, and task snapshot, then applies the draft's entries as task operations. That is the only path that persists tasks and can open Today.
...
[`src/mastra/agents/daily-planner.md`](../src/mastra/agents/daily-planner.md) is the canonical instruction contract for the single MVP planner. Build scripts embed it into the Mastra agent, and `npm run verify:planner` checks that the built instructions match the Markdown, that only the general planner is registered, and that it carries exactly the planner tools.

The agent can ask questions and change the draft through its tools. It cannot persist tasks itself or treat embedded task notes as instructions. Only Confirm commits changes. The Go API owns validation, authorization, concurrency, persistence, and all workflow transitions.
...
The UI runs rollover during account bootstrap, on a new local date, and on subsequent bootstrap refreshes. This is automatic when the app is used; no server scheduler runs while it is closed. Migration 10 supports carry-forward, 11 adds durable change receipts and review history, 12 adds conversation events, and 13 adds the day's draft and turn lock. Apply each migration before deploying the backend that needs it. The planner needs `CAPRIO_BACKEND_URL` and `PLANNER_TOOL_SECRET`; the backend needs the same `PLANNER_TOOL_SECRET`.
...
The conversation header shows `N remaining` beside View tasks. It counts unfinished work, excludes dropped tasks, and is absent for an empty day or unavailable workflow. Carry context moves into the conversation: a new day that is still being planned opens with an app-written line such as `Morning. 3 tasks carried over, 2 from yesterday. What's on today?` (no model call). The first committed turn stores that opener as the first entry of the day's thread.

The day's conversation is one continuous thread; it never collapses behind a draft. The app writes event entries into it — the opener, `Proposal discarded`, and `Plan saved · N tasks` — and passes them to the model as short `[Caprio]` notes so both see the same timeline. The plan card carries no model-written text. It shows the full resulting plan for the day, grouped into the day's tasks, Carried forward, and Other days, with a badge (New, Edited, Moved, Removed, Done) on each row that differs from what is saved, and two actions, Confirm plan and Discard. Under each reply that changed the plan, the app writes a `Plan updated · …` line. Revising happens by typing in the composer.
