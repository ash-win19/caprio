...
| `/new?date=YYYY-MM-DD` | Show the persisted conversation beside the saved checklist on desktop. Mobile shows a saved count and View tasks. | Add tasks or review suggestions. |
...
After rollover, opening the app or an undated `/today` bookmark uses the current day's persisted workflow state. A `planning` day opens the conversation at `/new`, including an empty day or one containing only carried tasks. An `active` or `closed` day opens Today. A failed workflow lookup offers retry before choosing a destination. Explicit dated links stay on their requested page, so View tasks remains available during planning and historical or future dates remain accessible.
