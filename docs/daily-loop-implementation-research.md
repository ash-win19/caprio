...
Preserve the typed cause and add stable, additive error codes. Keep the existing JSON `error` message and existing status conventions, so old clients continue to work. The canonical workflow codes are `plan_incomplete`, `over_capacity`, `validation`, `conflict`, `model_unavailable`, `not_found`, and `internal`. Authentication remains separate HTTP 401/403 handling; `auth` is the client-side expired-session category rather than a workflow service code.
