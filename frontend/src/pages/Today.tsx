...
function ContextPanel() {
  const { tasks, categories, carriedOver, streak } = useAppStore();
...
export default function Today() {
  const { tasks, setTasks, lastPrioritization, user } = useAppStore();
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over?.id) {
      ...
      setTasks(arrayMove(tasks, oldIndex, newIndex).map((t, i) => ({ ...t, order: i })));
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-lg font-medium text-foreground">Good morning, {user?.name || 'there'}</h1>
