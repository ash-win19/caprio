...
export const StatefulButton = forwardRef<HTMLButtonElement, StatefulButtonProps>(function StatefulButton(
  ...
  return (
    <Button ref={ref} disabled={isBusy || disabled} {...rest}>
      <span
        aria-live="polite"
      ...
      >
        <AnimatePresence mode="wait">
          {state === "loading" ? (
            <IconSlot keyId="loading">
              <Loader2 className="h-4 w-4 animate-spin" />
            </IconSlot>
          ) : null}
          {state === "success" ? (
            <IconSlot keyId="success">
              <Check className="h-4 w-4" />
            </IconSlot>
          ) : null}
          {state === "error" ? (
            <IconSlot keyId="error">
              <X className="h-4 w-4" />
            </IconSlot>
          ) : null}
        </AnimatePresence>

        <TextSlot value={textKey}>
          {stateText} 
        </TextSlot>

        <AnimatePresence>
          {state === "idle" && icon ? (
            <IconSlot keyId="icon">{icon}</IconSlot>
          ) : null}
        </AnimatePresence>
      </span>
    </Button>
